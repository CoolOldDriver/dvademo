...
class QuestionPoolPC extends React.Component {
  componentDidMount() {
  ...
  render() {
    ...
    return (
      <div className="poollist" >
         <Tabs defaultActiveKey="1" onChange={this.callback.bind(this)} size="large">
                { info
                 && info.map((res,index)=>{
                      return <TabPane
                              className="tab_three"
                              tab={
                                <div className={StyleSheet.aa}>{res}</div>
                              }
                              key={index+1}>
...
export default connect(mapStateToProps)(QuestionPoolPC);
